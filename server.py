...
def check_rate_limit() -> tuple[bool, str]:
    ...
    if _pro_key:
        return True, ""
    _usage_count["requests"] += 1
    if _usage_count["requests"] > FREE_LIMIT:
        msg = (
...
def _upsell_note() -> str:
    ...
    if _pro_key:
        return ""
    remaining = max(0, FREE_LIMIT - _usage_count["requests"])
    return (
...
async def _wiki_api(params: dict) -> dict:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        resp = await client.get(WIKI_BASE, params=params, headers={
            "User-Agent": "Wikipedia-MCP-Server/1.1 (AgentPay Labs; agent-wikipedia-tool)"
        })
...
async def wiki_search(query: str, limit: int = 10, language: str = "en",
    ...
    if not allowed:
        return json.dumps({"status": "error", "error": msg, "isError": True,
                           "next_steps": [f"Subscribe at {STRIPE_UPSELL}"]})
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            }, headers={"User-Agent": "Wikipedia-MCP-Server/1.1"})
            resp.raise_for_status()
            data = resp.json()
        ...
        if response_format == "markdown":
            ...
            md += _upsell_note()
            return md
        output["_upsell"] = _upsell_note().strip()
